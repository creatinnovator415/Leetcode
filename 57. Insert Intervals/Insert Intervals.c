...
int** insert(int** intervals, int intervalsSize, int* intervalsColSize, int* newInterval, int newIntervalSize, int* returnSize, int** returnColumnSizes) {
    ...
    merged_size = intervalsSize + 1;
    *returnSize = 0;
    *returnColumnSizes = (int*)malloc(sizeof(int)*merged_size);

    intervals = (int**)realloc(intervals, sizeof(int*) * merged_size );
    intervals[intervalsSize] = newInterval;

    
    quickSort(intervals, 0, merged_size-1);
    
    
    for(index = 1 ; index <= merged_size; index++)
    {
        if( (index == merged_size) || (false == _merge(intervals, base, index)) )
        {
